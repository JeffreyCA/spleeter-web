...
RUN apt-get -y install python3.9 python3.9-gdbm python3-distutils \
    ...
    && ln -s /usr/local/cuda-11.7/targets/x86_64-linux/lib/libcudart.so.11.0 /usr/lib/x86_64-linux-gnu/libcudart.so.11.0

RUN mkdir -p /webapp/media /webapp/staticfiles

WORKDIR /webapp
RUN pip3 install torch==1.13.1+cu117 torchaudio==0.13.1+cu117 -f https://download.pytorch.org/whl/torch_stable.html
